...
export default function FindTraveller() {
  ...
  return (
    <section className="mx-auto max-w-6xl px-6 py-16 lg:px-8">
      ...
      {/* Search form */}
      <div className="mt-10 rounded-3xl border border-ink-100 bg-white p-6 shadow-lg shadow-ink-900/5">
        <div className="grid grid-cols-1 items-end gap-4 md:grid-cols-[1fr_auto_1fr_1fr]">
          <div>
            ...
            <input
              ...
              className="mt-1 w-full rounded-xl border border-ink-200 px-4 py-2.5 text-sm text-ink-900 outline-none transition-colors focus:border-primary-600"
            />
          </div>
        ...
        <div className="mt-4 flex flex-wrap items-end gap-4">
          <div>
            <label className="text-xs font-medium text-ink-500">Transport</label>
            <select
              value={transport}
              onChange={(e) => setTransport(e.target.value)}
              className="mt-1 rounded-xl border border-ink-200 px-4 py-2.5 text-sm text-ink-900 outline-none focus:border-primary-600"
            >
              <option>Any</option>
          ...
          <div>
            <label className="text-xs font-medium text-ink-500">Min. weight needed (kg)</label>
            <input
              type="number"
              min="0"
              step="0.5"
              ...
              placeholder="0"
              className="mt-1 w-32 rounded-xl border border-ink-200 px-4 py-2.5 text-sm text-ink-900 outline-none focus:border-primary-600"
            />
          </div>

          <div>
            <label className="text-xs font-medium text-ink-500">Sort by</label>
            <select
              value={sortBy}
      ...
      <div className="mt-10">
        ...
        {results.length === 0 ? (
          <div className="mt-6 rounded-3xl border border-dashed border-ink-200 p-12 text-center">
            <p className="font-display text-lg font-semibold text-ink-900">No trips match yet</p>
